  ...
  return (
    <div className="page">
      ...
      <section className="grid-main">
        <div className="card"><div className="card-header"><div><h2>Sample registrations</h2><p>New samples added this week</p></div><Badge tone="success">+18.2%</Badge></div><div className="card-body"><div className="chart-box"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData.registrations}><defs><linearGradient id="sampleFill" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--primary)" stopOpacity={0.24}/><stop offset="95%" stopColor="var(--primary)" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--line)"/><XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: "var(--muted)", fontSize: 11 }}/><YAxis axisLine={false} tickLine={false} tick={{ fill: "var(--muted)", fontSize: 11 }}/><Tooltip contentStyle={{ border: "1px solid var(--line)", borderRadius: 8, background: "var(--surface)", color: "var(--ink)" }}/><Area type="monotone" dataKey="samples" stroke="var(--primary)" strokeWidth={2.25} fill="url(#sampleFill)" /></AreaChart></ResponsiveContainer></div></div></div>
      ...
      <section className="grid-3">
        <div className="card"><div className="card-header"><div><h2>Equipment usage</h2><p>Utilization over the last 7 days</p></div></div><div className="card-body"><div className="chart-box" style={{ height: 220 }}><ResponsiveContainer><BarChart data={chartData.equipmentUsage}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--line)"/><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: "var(--muted)", fontSize: 9 }}/><YAxis axisLine={false} tickLine={false} tick={{ fill: "var(--muted)", fontSize: 10 }}/><Tooltip cursor={{ fill: "var(--surface-3)" }} contentStyle={{ borderRadius: 8, border: "1px solid var(--line)", background: "var(--surface)", color: "var(--ink)" }}/><Bar dataKey="usage" fill="var(--primary)" radius={[4,4,0,0]} /></BarChart></ResponsiveContainer></div></div></div>
